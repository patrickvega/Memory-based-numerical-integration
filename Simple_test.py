...
class loss(keras.layers.Layer):
    ...
    def call(self,inputs):
        ...
        # x = tf.random.uniform([int(0.5 * self.n_pts)],
        #                       dtype = dtype, 
        #                       maxval = 1)
        ...
        ## u -x*sin(10x) = 0 is eq, PDE loss
        error_PDE = keras.ops.mean((self.u_model(x, training = True) - self.f(x))**2)
        
        ##Dirichlet BC loss
        # bc = self.u_model(np.array([0.]))**2 + self.u_model(np.array([1.]))**2
        #return error_PDE + bc
        return error_PDE
...
xlist = np.array([1/1000 * i for i in range(1000)])
